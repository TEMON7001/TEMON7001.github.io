...
// прямо в том же Chrome, где живёт приложение.
...
export function eqBytes(actual, expected, what = "байты") {
  const a = Array.from(actual);
  const b = Array.from(expected);
  if (a.length !== b.length || a.some((v, i) => v !== b[i])) {
    throw new Error(what + ": получено [" + a.join(" ") + "], ожидалось [" + b.join(" ") + "]");
  }
}
...
export function report(root) {
  ...
  document.title = (failed.length ? "✗ " : "✓ ") + "Тесты CAN-дешифратора";
